...
| 📋 プロファイル | **1プロファイル = 1バックエンド**。編集画面で LLM バックエンドをプルダウンで選ぶと、そのプロジェクトの**起動パラメータ一式がフォームに表示され、既定値もプリフィル済み** — 変更する項目だけ触れば完成。追加引数 (生フラグ)・環境変数・高度なコマンド上書きも可。保存前に実行コマンドをリアルタイムプレビュー |
...
2. 選んだ瞬間に、そのバックエンド専用のパラメータフォームへ切り替わる (例: vLLM なら `--tensor-parallel-size` / `--gpu-memory-utilization`、llama.cpp なら `--ctx-size` / `--n-gpu-layers`)
3. 各項目には推奨既定値がすでに入力済み (ポート・host・TP=1・dtype=auto など)。プレビューを見ながら変更する項目だけ編集
4. 空欄にした項目はフラグ自体が付かない (バックエンド本来のデフォルトに委ねられる)
...
- 空欄の項目はフラグ自体が付きません
- 「コマンドのみモード」(高度な設定): ON にするとフォーム由来のフラグ自動付与を止めます
- 旧 v2 (1プロファイルに複数バックエンド) の `data/profiles.json` は、起動時に**バックエンドごとの別プロファイルへ自動分離**されます (2件目以降の名前には `[vLLM]` 等の接尾辞が付きます)。API も単一バックエンド形式が正規形で、旧 configs ボディは 1バックエンドへ平坦化して受理します
